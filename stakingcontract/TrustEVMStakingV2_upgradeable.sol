...
contract TrustEVMStaking is AccessControlEnumerable, Initializable {
    ...
    function initialize(address _adminAccount, uint256 _evmRewardsPerBlock, uint256 _evmStakingPeriodLength) public initializer {
    ...
    function pendingEvm(address _user) external view returns (uint256)
    {
        uint256 accumulatedEvmPerShare = stakingPool.accumulatedEvmPerShare;
        ...
        if (block.number > stakingPool.lastRewardBlock && stakingPool.balance != 0) {
            ...
            accumulatedEvmPerShare += evmReward * 1e12 / stakingPool.balance;
        }

        return (stakingUserInfo[_user].amount * accumulatedEvmPerShare / 1e12) - (stakingUserInfo[_user].rewardDebt - stakingUserInfo[_user].owedAmount);
    }

    function getOwedAmount(address _user) external view returns (uint) 
    {
        return stakingUserInfo[_user].owedAmount;
    }
    ...
    function stake() public payable onlyStaker
    {
        ...
        if (stakingUserInfo[msg.sender].amount > 0) {
            ...
            stakingUserInfo[msg.sender].owedAmount += pending;
        }
    ...
    function withdraw(uint256 _amount) public onlyStaker
    {
        ...
        // Substract the 'owed rewards' to rewardDebt to send them over with the 'current Rewards'
    ...
    // Staker can claim rewards when:
    //     1- Staked amount is 0 or greater.
    //     2- Staking period is over.
    //     3- Owed amount is greater than 0.
