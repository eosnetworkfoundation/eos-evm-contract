...
bool post_check(const InMemoryState& state, const nlohmann::json& expected, std::vector<evmc::address>& coinbases) {
    ...
    for (const auto& entry : expected.items()) {
        ...
        if (account->balance + coinbaseCount * 2000000000000000000 != expected_balance) {
...
RunResults blockchain_test(const nlohmann::json& json_test) {
    ...
    std::vector<evmc::address > coinbases;
    for (const auto& json_block : json_test["blocks"]) {
        Status status{run_block(json_block, blockchain)};
        if (status != Status::kPassed) {
            return status;
        }
        std::string coinbaseAddr = json_block["blockHeader"]["coinbase"];
        evmc::address coinbase{to_evmc_address(*from_hex(coinbaseAddr))};
        coinbases.push_back(coinbase);
    }
...
*/
    ...
    if (post_check(state, json_test["postState"], coinbases)) {
        return Status::kPassed;
    } else {
        return Status::kFailed;
    }
