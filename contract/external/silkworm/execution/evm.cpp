...
CallResult EVM::execute(const Transaction& txn, uint64_t gas) noexcept {
    eosio::check(txn.from.has_value(), "no from");  // sender must be recovered
...
evmc::result EVM::execute(const evmc_message& msg, ByteView code, std::optional<evmc::bytes32> code_hash) noexcept {
    ...
    evmc_result res = execute_with_baseline_interpreter(rev, msg, code);
...
evmc_tx_context EvmHost::get_tx_context() const noexcept {
    ...
    eosio::check(header.number <= INT64_MAX, "header.number <= INT64_MAX");  // EIP-1985
    context.block_number = static_cast<int64_t>(header.number);
    eosio::check(header.timestamp <= INT64_MAX, "header.timestamp <= INT64_MAX");  // EIP-1985
    context.block_timestamp = static_cast<int64_t>(header.timestamp);
    eosio::check(header.gas_limit <= INT64_MAX, "header.gas_limit <= INT64_MAX");  // EIP-1985
...
evmc::bytes32 EvmHost::get_block_hash(int64_t n) const noexcept {
    ...
    eosio::check(new_size <= 256, "new_size <= 256");
