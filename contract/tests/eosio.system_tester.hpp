...
public:
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      return get_balance("eosio.token"_n, act, balance_symbol);
   }
   ...
   void issue( const asset& amount, const name& manager = config::system_account_name ) {
      issue("eosio.token"_n, amount, manager);
   }

   void issue( name contract, const asset& amount, const name& manager = config::system_account_name ) {
      base_tester::push_action( contract, "issue"_n, manager, mutable_variant_object()
                                ("to",       manager )
                                ("quantity", amount )
                                ("memo",     "")
                                );
   ...
   void transfer( const name& contract, const name& from, const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      base_tester::push_action( contract, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   void transfer( const name& from, const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      transfer("eosio.token"_n, from, to, amount, manager);
   }
   ...
   fc::variant get_stats( name contract, const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( contract, name(symbol_code), "stat"_n, account_name(symbol_code) );
      return data.empty() ? fc::variant() : token_abi_ser.binary_to_variant( "currency_stats", data, abi_serializer_max_time );
   }

   fc::variant get_stats( const string& symbolname ) {
      return get_stats("eosio.token"_n, symbolname);
   }
